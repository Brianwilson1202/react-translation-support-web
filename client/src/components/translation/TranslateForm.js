...
const FIELDS = [
  ...
  { label: "TempField", name: "tempField", error: "This is temp component" },
...
class TranslateForm extends React.Component {
  renderFields() {
    return _.map(FIELDS, ({ name, label }) => {
      switch (name) {
        case "tempField":
          return <DropDownMenu key={name} />;
        case "language":
          return <LanguageDrop key={name} label={label} />;
        // case 'language':
        //     return(
        ...
        case "tags":
          return <NewTag key={name} label={label} />;
        default:
          return (
            <Field
              key={name}
